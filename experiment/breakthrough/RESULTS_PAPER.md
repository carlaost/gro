...
**The apparent paradox — how can the panels agree at 0.89 yet the metric agree more with Claude (0.58) than GPT (0.34)?** No contradiction: it is the signature of shared-method variance. Decompose each judge's scores as `CONSENSUS + model_quirk + noise`. The panels correlate at 0.89 because CONSENSUS dominates — the two model families largely agree on what a breakthrough is. But the metric is a Claude compiler feature, so it carries `CONSENSUS + Claude_quirk`. Correlated against Claude it collects CONSENSUS *plus* the shared quirk (0.58); against GPT it collects CONSENSUS only, since GPT's quirk is uncorrelated with a Claude feature (0.34). The ≈0.24 gap *is* that shared Claude idiosyncrasy. The tell: were the metric a *strong* predictor (approaching the panels' own agreement), the two correlations would converge on CONSENSUS; it is precisely because the metric is weak that the quirk term is a large fraction of its correlation and the gap opens. So the honest signal is the cross-model number, 0.34, and the gap quantifies the bias rather than contradicting the panel agreement.

So the metric survives the bias test — but diminished. It is a genuine, transferable-but-weak triage signal (~0.34 cross-model), not the ~0.57 the same-family setup suggested. The remaining validation — a historical/longitudinal corpus whose ground truth is what the field actually did next (§10) — would replace the LLM panel with the world, removing the shared-method concern entirely.
