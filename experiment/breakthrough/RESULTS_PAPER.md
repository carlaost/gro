...
**Headline:** the best metric is a single feature — the compiler's typed contribution-depth, `max(peak, cwmean)` — and its apparent skill is **largely shared-model bias**. It tracks a same-family LLM expert panel at ρ ≈ 0.58 (held-out) but an *independent* model family at only ρ ≈ 0.34 (the lean flips when the other model builds the metric; Steiger p = 0.003). Structural features (prior-art overlap, delta ledger, genre) add no transferable signal. The decisive real-world test — does the publication-time metric predict which papers actually reshaped the field? — is a **separate follow-on study now in progress** (§10): a longitudinal/historical validation against mature citation-disruption, with a preliminary abstract-only run already returning a null that we are strengthening with full text.
...
**The follow-on test (§10):** the decisive real-world validation — removing the LLM from the ground truth by predicting *mature field disruption* of historical papers from publication-time text — is promoted to a **separate longitudinal study, now in progress**. Its preliminary abstract-only pass returns a null (ρ near zero), directionally consistent with §9, but is under-powered by design and being strengthened with full text; it is the motivating result for that study, not this paper's verdict. This paper's deliverable is a **triage prior that flags LLM-perceived contribution depth** (transferable skill ~0.34, not the 0.58 a same-family panel advertises) plus the diagnosis of why (shared-method bias, §9).
...
## 10. The decisive test, as a follow-on study — does the metric predict what the field actually did?

Every number above is the metric agreeing with an *LLM's opinion*. The decisive test removes the LLM from the ground truth entirely and is being written up **as a separate follow-on paper** (longitudinal/historical validation); this section states its design and the preliminary result. Take **historical** papers, compute the metric from the paper text *at publication time* (no hindsight), and check whether it predicts which papers actually **reshaped the field** — measured by mature disruption on the now-complete citation graph.
...
**Preliminary result: the publication-time metric does not (yet) predict mature disruption.** Spearman ρ hovers around zero and is *not sign-stable* across citer-reliability thresholds — −0.13 (n=62, citers≥10), +0.23 (n=37, ≥30), +0.16 (n=23, ≥50); top-metric-quartile papers were if anything slightly *less* disruptive than the bottom (mean mDI −0.52 vs −0.39). It predicts total citations no better (ρ ≈ −0.06). **No robust signal in either direction — on a deliberately weak first pass** (see caveat below).

Directionally this is what the shared-method-bias analysis (§9) predicts: if much of the 0.58 panel agreement is two LLMs sharing a prior about what *sounds* important, a real-world target should deflate it. But we flag this as **preliminary, not the paper's verdict**, because the current pass is under-powered by design (abstract-only typing, n=62, single domain) — hence its promotion to a dedicated follow-on study with full text and multiple domains.

**Rigor.** Bootstrap over the n=62 papers puts the correlation at **ρ = −0.13, 95% CI [−0.38, +0.14]** — the interval straddles zero, so there is no evidence of a relationship in either direction (not merely a small one). A documented-landmark cross-check was attempted but came out degenerate for an instructive reason: in this corpus mature **disruption and citation count anti-correlate** (ρ ≈ −0.18) — consolidating work accrues more citations than disruptive work, the classic Funk–Owen-Smith pattern — so "highly-cited *and* disruptive" is nearly an empty set (n=1), and the publication-time metric predicts *neither* axis (vs citations ρ ≈ −0.06).

**The main caveat — full text — is being removed now (the reason this is a follow-on).** The compressed-variance worry is real: abstract-only typing gives the predictor almost no spread (23 unique values across 62 papers, a pile-up at 0.70), which attenuates any correlation. The fix is to type contributions from *full text*. 2004–2010 papers are heavily paywalled — automated OA fetch (Unpaywall PDF, publisher HTML, Europe PMC XML) reaches mostly the well-deposited, higher-cited papers and misses the low-cite contrast class — so full-text assembly is an active, partly-manual effort underway for the follow-on. (That access asymmetry is itself a finding: the substrate's premise, "compile the literature," is access-blocked for exactly the historical control papers a discrimination test needs.)

**So the honest status:** the LLM-free test currently shows *no evidence* that the metric predicts field step-changes, but on a deliberately weak first pass; it is **not the paper's conclusion** but the motivating null for a dedicated longitudinal validation (full text, multiple domains, disruption as the target). This paper's verdict is the shared-method-bias result (§9): the metric's real, transferable skill is ~0.34, not the 0.58 a same-family LLM panel advertises.
...
- Against **real-world outcomes** (historical papers vs mature field disruption, no LLM in the ground truth) a preliminary pass shows no signal — but that test is promoted to a **separate follow-on study** with full text before any verdict (§10).
