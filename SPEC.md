...
**Two metrics from one measurement (breakthrough vs convergence).** `overlap` reads with opposite sign for two distinct research virtues, and downstream metrics MUST declare which they compute: **breakthrough / novelty = `1 − overlap`** (distance from prior work); **convergence / consolidation = `+ overlap`** (proximity to, and pulling-together of, prior work — a synthesis or unifying framework). A record can score high on one and low on the other. The breakthrough reading is validated (ρ≈0.57 vs a SOTA-grounded expert panel, held-out); the convergence reading is specified here but awaits its own labeled validation.
